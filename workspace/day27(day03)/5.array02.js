...
// .slice(시작, 끝) : 원하는 부분을 추출하기 위해서 시작 인데스와 마지막 인데스를 전달한다
// 마지막 인덱스는 포함되지 않는다
...
console.log(number2[0], typeof number2[0]); //number 타입으로 출력 //1 number

//슬라이싱 : 여러개의 값을 추출할 때 사용
console.log(number2.slice(0,1), typeof number2.slice(0,1)); //object 배열타입으로 출력 // [ 1 ] object
console.log(number2.slice(2,8), typeof number2.slice(2,8)); //[ 3, 4, 5, 6, 7, 8 ] object
//console.log(number2.slice[(2,5)]); //undefined
//slice[(2,5)] => slice[5] 처럼 접근하는 것으로 해석되는데
//함수가 배열로 접근하기 때문에 undefined로 출력된다
//slice는 두 개의 정수를 매개변수로 받는다 [2,5] 첫번째 매개변수로 처리되고 두번째 매개변수는 undefined로 처리된다

//.pop() : 마지막 요소를 삭제되는 값을 반환
console.log(number2.pop()); //8
...
//.splice(인덱스번호, 개수) : 인덱스번호삭제(무조건 2개 매개변수로 작성)
...
console.log(numbers2.indexOf(2));//1 인덱스 반환

console.log(numbers2.indexOf(3));// 2인덱스 반환
